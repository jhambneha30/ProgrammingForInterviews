...
Instead of having all the nodes in the below linked list, keep replacing the first node of the LL with the new node at that distance. Then at the end, we will have the lowest node at that particular horizontal distance and these will form the bottom view of the binary tree.

TC=O(n)
SC=O(n) for stack and O(n) for DLL structure in worst case
*/
...
struct DLLnode* verticalOrderTraversal(struct node *root,struct DLLnode *head, int hd,char flag)
{
	...
	if(flag=='l')
	{
		...
		{
			head->prev->below->data=root->data;
	...
	if(flag=='r')
	{
		...
		{
			head->next->below->data=root->data;
